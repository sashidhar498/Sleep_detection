...
class face_points:
    def __init__(self):
        ...
        pass
    def find_list_with_largest_area(self,coordinate_lists):
        largest_area = 0
        largest_list = None

        for coordinates in coordinate_lists:
            if len(coordinates) != 4:
                continue  # Ensure there are exactly four elements in the list
    ...
    def points(self,frame):
        ...
        faces=[]
        for rect in rects:
            faces +=[face_utils.rect_to_bb(rect)]
        
            # compute the bounding box of the face and draw it on the
            # frame
        print(rects)
        try:
            bX, bY, bW, bH=(self.find_list_with_largest_area(faces))
